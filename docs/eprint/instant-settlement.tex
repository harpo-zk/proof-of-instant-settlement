...
\author{Juliano P. Sales \and Marco T\'ulio Rocha Nascimento\\
\small Harpo \quad\textbar\quad \texttt{harpo-zk}\\
\small \href{mailto:tzdesing@gmail.com}{tzdesing@gmail.com} \quad\textbar\quad \href{mailto:marcotulio.nascimento@gmail.com}{marcotulio.nascimento@gmail.com}}
...
\begin{abstract}
On-chain confidentiality layers (Zeto, Railgun, Rayls/Enygma) prove
\emph{confidential transfers} between on-chain accounts, but within the
confidential-transfer family on public EVM chains none of them binds its private
state to a \emph{real fiat settlement}. For tokenized-asset settlement integrated
with instant-payment rails --- concretely Brazil's central-bank instant-payment
system --- the question that matters is not ``did an on-chain balance move'' but
``\emph{did the fiat money actually settle, for this exact operation}'', and to
answer it \emph{without revealing} amount or counterparties. We present
Proof-of-Instant-Settlement, a construction that cryptographically binds a
specific instant payment, identified by its rail-level \emph{EndToEndId}
(E2EID) and \emph{attested by a regulated PSP}, to an on-chain commitment via a
Groth16 circuit, together with an attestation oracle that lets the chain
verify --- through an EIP-712 signature from a registered payment service
provider (PSP) --- that the E2EID corresponds to a payment the PSP reports as
settled and credited. We are precise about what is proved and by whom: the
...
(Assumption~\ref{as:psp}). \emph{Not proved}: that the central bank's own
...
states the reference artifacts' setup as development-grade).
\end{minipage}}
\end{center}
...
assumption (Assumption~\ref{as:psp}), stated as the central fact about this
...
PSP's \eeid{}$\to$\code{txid} bijection (Assumption~\ref{as:psp}).
...
\eeid{}$\to$\code{txid} bijection (Assumption~\ref{as:psp}). A deployment that
...
exactly where the unconditional guarantee ends and Assumption~\ref{as:psp}
begins.
...
and \code{finalize}$(\pi)$ verifies it, cross-checks the oracle attestation for
$\nu$ (same $c_0$ and same $\mathsf{pc}$), checks the domain anchor $K$, consumes
the tag, and transitions the operation to \textsc{Finalized}. It becomes
...
signed receipt can be used, containing the damage from a compromised PSP key. The
receipt additionally carries $\mathsf{pc}$, the counterparty commitment of
...
permanently burns that $\nu$: the payment can no longer close its operation, which
then falls into the liveness gap of \S\ref{sec:security} (escrow timeout,
off-chain dispute). We state this as a property of the anti-replay choice rather
...
paid --- under Assumption~\ref{as:psp}: a PSP that signs a wrong $\mathsf{pc}$ is
...
it. We validated on-chain that a \emph{genuinely valid} Groth16 proof for a
...
and predicate circuits --- for which we give proving time and on-chain
verification gas --- the full in-circuit ML-KEM binding has no end-to-end
...
We give a syntax for the construction and state its properties as reductions to
standard assumptions. Let $\Pi=(\mathsf{Setup},\mathsf{Commit},\mathsf{Prove},
...
uniqueness). (A4) is what lets \code{txid} do the anti-cross-charge work
...
\code{txid} appears in both commitments. The assumptions above are not all the
...
Trusted PSP (Assumption~\ref{as:psp}) & institutional & false \code{settled} accepted \\
Trusted setup (below) & cryptographic, contingent on process & forged proofs, if toxic waste leaked \\
...
\label{prop:twophase}
For any accepted proof, the amount and \code{txid} witnessed in $c$ are
identical to the amount and \code{txid} witnessed in $c_0$.
\end{property}
\begin{proof}[Proof sketch]
By (A1) an accepting prover knows a witness satisfying (3)--(7); amount and
\code{txid} are the \emph{same witness signals} in (3) and (4) --- not merely
constrained to be equal, but literally one circuit signal read twice --- so a
mismatch between the two is not expressible in an accepting witness at all.
This is what an escrow locked on $c_0$ relies on for the terms it locked to be
the terms that settled.
\end{proof}

\begin{property}[2C --- Settlement binding]
\label{prop:settlementbinding}
An accepted settlement's tag $\nu$ and counterparty commitment $\mathsf{pc}$
are the ones a registered PSP attested for this $c_0$
(Property~\ref{prop:auth}), and $\nu=\Poseidon(\eeid,k)$ for the \eeid{}
witnessed in $c$ under the domain's registered key.
\end{property}
\begin{proof}[Proof sketch]
$\mathsf{Settle}$'s cross-check (\S\ref{sec:construction},
\S\ref{sec:oracle}) requires the oracle's stored binding for $\nu$ to equal
$\mathsf{keccak256}(c_0\Vert\mathsf{pc})$; by (A3) that binding could only be
recorded by a registered PSP's signature over exactly this $(c_0,\mathsf{pc})$
pair. The tag equation is Equation~(5), checked by the same argument as
Property~\ref{prop:twophase}.
\end{proof}
...
it routes through Assumption~\ref{as:psp} for what the attestation
...
\begin{property}[On-chain confidentiality under the Poseidon random-oracle model]
\label{prop:conf}
The public transcript --- $(c_0,c,\nu,K,\mathsf{pc},\pi)$ \emph{together with the
calldata of} \code{attest} \emph{and} \code{settle}, i.e. including
$\mathsf{Receipt}(c_0,\nu,\mathsf{settled},\mathsf{deadline})$ --- is simulatable
without $(\text{amount},\eeid,\text{party},\text{txid})$, revealing them only with
negligible advantage. The property is scoped to \emph{on-chain observers without
...
an oversight). The hiding argument below is a random-oracle heuristic for
Poseidon (A2), not a reduction to a standalone computational hardness
...
The transcript is stated to include the receipt and the calldata deliberately:
a transcript restricted to $(c,\nu,\pi)$ alone would be insufficient --- any
receipt design whose signed fields carry the amount or \code{txid} in the
clear, as \S\ref{sec:oracle} discusses, falsifies the property outright, since
that calldata is exactly as public and permanent as the settlement transcript
itself. We do not construct a single joint simulator for the whole transcript;
instead we argue each public element's hiding independently, which suffices
because the elements below are exactly the transcript's public elements and
none is a function of the others computable without the hidden witness.
...
identifying the domain. Under the random-oracle model, each image is
independently indistinguishable from a uniformly random field element to
anyone without its corresponding blinding factor. \emph{(C) Metadata is
...
protected ones is this property's subject. \emph{(D) The receipt's EIP-712
signature is authentication, not an added privacy mechanism.} The signature
over $\mathsf{Receipt}(c_0,\nu,\mathsf{settled},\mathsf{deadline})$ proves
which registered key signed (A3, Property~\ref{prop:auth}); it neither hides
nor further reveals $c_0$ or $\nu$ beyond what (B) already established for
them as public signals --- the receipt and the settlement transcript were
already joined through $\nu$, so the signature adds an authentication fact,
not a new information leak. Composing (A)--(D): every public element of the
stated transcript is independently simulatable or already-public without the
excluded values, and (C) delimits exactly what this composition does not
claim.
\end{proof}

\begin{property}[Domain-scoped settlement uniqueness]
\label{prop:uniqueness}
Within a domain under a fixed, immutable key $K$, and under
Assumption~\ref{as:psp}'s PSP \eeid{}$\to$\code{txid} bijection, each settled
payment closes at most one operation. The property is explicitly
\emph{domain-scoped}: it neither states nor implies uniqueness across domains
(distinct $K$, including a rotated one) without that same assumption.
\end{property}
\begin{proof}[Proof sketch]
$\nu$ is deterministic in \eeid{} given $k$ --- determinism in \eeid{} is (5) ---
and the public anchor $K$ forces all accepted proofs in a domain to use the same
$k$ (6); the oracle and the settlement each record the tag as single-use, so a
second settlement of the same \eeid{} presents an already-recorded tag and is
rejected. Cross-domain uniqueness is not implied by the circuit and rests on the
PSP's \eeid{}$\to$\code{txid} bijection (Assumption~\ref{as:psp}). Uniqueness is
furthermore \emph{per domain key}: $K$ is immutable per domain
(\S\ref{sec:construction}), so rotation means a new domain, across which ---
exactly as across domains in general --- uniqueness rests on
Assumption~\ref{as:psp}.
\end{proof}
...
is exactly the residual trust of Assumption~\ref{as:psp}.
\end{property}
\begin{proof}[Proof sketch]
...
the reference artifacts meet at development grade only (the trusted-setup
assumption of \S\ref{sec:security}). We now enumerate adversaries informally,
...
$\mathsf{pc}$ is Assumption~\ref{as:psp}. \\[2pt]
...
domain, and cross-domain uniqueness rests on Assumption~\ref{as:psp}
...
window of abuse, and the attestor can be revoked for future attestations. A
compromised key remains able to attest within the window. \\[2pt]
...
Settlement (credit availability) actually happened & PSP attestation alone (Assumption~\ref{as:psp}) \\
...
development grade explicitly rather than in a footnote. \\
Registry owner key (policy admin) & Add a hostile attestor or rewrite policy
roots; mitigated only by a production multisig-with-timelock, not by this
prototype's single key. \\
\bottomrule
\end{tabular}
\end{center}

\begin{assumption}[Trusted PSP]
\label{as:psp}
...
\begin{assumption}[Trusted setup]
Groth16 soundness holds provided the per-circuit setup was performed such that
the toxic waste was discarded. The reference artifacts use a development-grade
setup; a production deployment requires a multi-party ceremony. This is a known
...
Assumption~\ref{as:psp} stops meaning \emph{trusted} PSP and starts meaning
...
and the payment identifiers (\eeid, txid): all appear only as Poseidon images,
with negligible leakage under (A2). It does \emph{not} hide: (i) the
...
the same one. The primary circuit has $1{,}260$ wires and $4{,}229$ labels,
...
\midrule
\code{settlement\_verify\_v2} & $151.2$ ms & $13.6$ ms & $\mathbf{243{,}194}$ \\
\code{pix\_nullifier\_verify}      & $110.5$ ms & $14.0$ ms & --- \\
\code{pix\_payment\_verify} (ablation) & $130.1$ ms & $14.7$ ms & $238{,}387$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent We deliberately do \emph{not} quote a measured delta against the
two-signal ablation. The two verifiers ($238{,}387$ gas for
\code{pix\_payment\_verify}, $243{,}194$ for \code{settlement\_verify\_v2}) were
generated and measured under conditions we cannot certify identical --- distinct
snarkjs invocations, potentially distinct trusted-setup runs --- so the two
figures are not a controlled A/B on signal count alone. As a sanity check: the
na\"ive difference between them ($4{,}807$ gas) is \emph{smaller} than the cost
three extra public signals must add on their own under EIP-1108 (one
\code{alt\_bn128} scalar multiplication plus one addition per signal, $\approx
6{,}150$ gas each, $\approx 18{,}500$ for three), which confirms the two
measurements are not directly comparable rather than telling us what the true
per-signal delta is; we do not report a delta figure, measured or expected, until
a same-template re-measurement (\S\ref{sec:futurework}) produces one we would
stand behind. The dominant cost of either verifier remains the four-pairing check
...
\midrule
\code{lock(c0)} --- escrow locks on the agreed terms & $73{,}936$ \\
\code{attest(receipt, signature)} --- ECDSA recover, checks, tag storage (v2: $88{,}646$) & $90{,}913$ \\
\textbf{\code{finalize(proof)} --- complete settlement (v2: $352{,}983$)} & $\mathbf{353{,}322}$ \\
\quad of which \code{verifyProof} (Groth16, 5 public signals) & $243{,}194$ \\
\code{attestReversal(reversal, signature)} & $57{,}553$ \\
\bottomrule
\end{tabular}
\end{center}

\noindent Figures are v3 (with $\mathsf{pc}$ in the receipt and the
\code{finalize} cross-check); v2 figures are given alongside for the two calls
whose struct changed. \code{attest} costs $2{,}267$ gas more than v2 (one extra
bytes32 field in calldata and in the EIP-712 digest); \code{finalize} costs
$339$ gas more (one \code{keccak256} over 64 bytes plus the extra calldata word
for the cross-check) --- both consistent with the single-word binding storage of
\S\ref{sec:oracle} rather than a fresh storage slot. The complete settlement
transaction costs $353{,}322$ gas. An \emph{isolated} call to the verifier
contract with the same proof and public signals costs $243{,}194$ gas ---
including its own $21{,}000$ base transaction cost and its own proof calldata,
neither of which recurs when the verifier is instead called \emph{internally}
from \code{finalize}, so $243{,}194$ is an upper bound on the verifier's share of
the $353{,}322$, not a component that subtracts cleanly to give the remainder.
With that caveat, the bulk of the complete transaction's cost is still the
Groth16 verification; the rest is the oracle cross-check (the receipt must be
attested \emph{and} carry the same $(c_0,\mathsf{pc})$ the escrow and the proof
agree on), the domain-anchor check, the tag consumption and the state
transition. Quoting the isolated verifier figure alone, unqualified, would both
overstate its call-time cost and understate the complete transaction's real
cost --- which is why we report the complete-call figure as primary and give the
isolated figure only with this caveat attached.
...
circuit is otherwise identical --- same source, same $1{,}251$ constraints at
\code{--O2}. Under this methodology we obtain genuine, non-vacuous results:
...
\midrule
\code{settlement\_verify\_v2} (\code{--O0}) & declared & $5/5$ & $4{,}229/4{,}229$ & sound, no trusted fns \\
\code{kyc\_inclusion\_verify} (\code{--O0}) & declared & $1/1$ & $18{,}230/18{,}248$ & sound, no trusted fns \\
\code{settlement\_verify\_v2} (\code{--O2}) & declared & $4/5$ & $1{,}016/1{,}260$ & \emph{inconclusive} \\
\bottomrule
\end{tabular}
\end{center}
...
nine cases were submitted with explicit gas so that they would be recorded on
the chain rather than stopped by client-side simulation, leaving a hash a
reader can look up.
...
[TODO: URL], at commit [TODO: full hash]. Reproducibility claims in this
section are claims about that exact commit.
...
($\mathtt{0x7c20}\ldots\mathtt{a49}$) --- a prototype convenience, not a design
property. The negative cases must be read accordingly: the non-attestor case is
signed by a second key, $\mathtt{0x19E7}\ldots\mathtt{ff2A}$, and a production
...
Groth16/Poseidon/BabyJubJub primitives, central-bank pilots. It is a two-tier
private-ledger design with a regulator commit-chain, and its auditor holds a
\emph{global} view key. Zeto~\cite{zeto} and Nightfall provide generic
confidential transfers on EVM. To the best of our knowledge, none of these binds
a real instant-payment rail in the sense evaluated here: they answer ``was the
on-chain transfer well-formed'', not ``did the fiat settle''. Our contribution
is orthogonal to, and composable with, those systems.
...
scripts are available in the reference repository, [TODO: URL], at commit
[TODO: hash], together with the artifact manifest described in
\S\ref{sec:impl}. This is a research prototype validated on a public testnet;
